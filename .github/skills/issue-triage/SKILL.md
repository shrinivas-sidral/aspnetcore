...
name: issue-triage
description: >-
  Classify or triage a dotnet/aspnetcore GitHub issue by its single best area, issue type
  (Bug, Feature, Task, or Epic), supported subtype, duplicate status, abstention, and
  triage-summary recommendation. USE FOR requests to classify an ASP.NET Core issue,
  recommend triage metadata, distinguish Bug from Task, identify the owning ASP.NET Core
  area, assess a supplied duplicate candidate, or draft a triage summary. DO NOT USE FOR
  implementing an issue, investigating or fixing a bug, reviewing a pull request, or
...
2. Exactly one issue type: `Bug`, `Feature`, `Task`, or `Epic`.
...
| `Epic` | A cross-cutting umbrella effort intended to coordinate multiple independently deliverable issues. A single large feature request is not automatically an Epic. |

Trust an issue-template signal only when the issue content supports it.
...
(or abstention), type, subtype (or none), and duplicate decision. Then draft the summary
...
**Type:** `Bug` | `Feature` | `Task` | `Epic` (brief evidence-based reason)
